...
/**
 * Classe CritereSecteurActivite représentant la table critere de la base de
 * données
 */
public class CritereSecteurActivite implements CritereInterface, Cloneable,
		Serializable {
	...
	private int id = 0;
	private final String typeCritere = "Secteur activité";
	private final Comparateur comparateur = null;
	private SecteurActivite valeurComparee = null;

	public CritereSecteurActivite() {
	}

	public CritereSecteurActivite(int id) {
		this.id = id;
	}
	...
	public void setValeurComparee(String valeurComparee) {
		this.valeurComparee = SecteurActivite.valueOf(valeurComparee);
	}
	...
	public boolean controler(DossierIndemnisation dos) {
		return dos.getAttestation().getEmployeur().getSecteurActivite() == this.valeurComparee;
	}
	...
	public boolean isEgal(CritereInterface objet) {
		...
		if (this.valeurComparee == null) {
			if (objet.getValeurComparee() != null)
				return (false);
		} else if (!this.valeurComparee.equals(SecteurActivite.valueOf(objet
				.getValeurComparee()))) {
			return (false);
		}
		return true;
	...
	public String toString() {
		return "Secteur : " + this.valeurComparee.getNom();
	}
	 ...
	 */
	public Object copie() {
		CritereSecteurActivite copie = new CritereSecteurActivite();
		copie.setId(this.id);
		if (this.typeCritere != null)
